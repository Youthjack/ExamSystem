$(document).ready(function() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			var fill_questions = [];
			for(var i=0; i<num; i++) {
				if(d[i.toString()]['type'] == '0') {
					choice_questions.push(i);
					total_socre += d[i.toString()]['point'];
				}
				if(d[i.toString()]['type'] == '1') {
					fill_questions.push(i);
					total_socre += parseInt(d[i.toString()]['point']);
				}
			}
	...
	$('#exam_submit').click(function() {
		...
		//var ajaxUrl = 'http://119.29.4.91:8080/student/finishExam'
		$.ajax({
			url: ajaxUrl,
			type: 'get',
			data: {paperId: pid, 
